...
**Claude 會自動識別這是 create 操作，詢問 spec 路徑**
...
**Claude 會自動識別這是 update 操作，詢問 test file 路徑**
...
| 場景 | NLP 說法 | 手動指令 |
|------|---------|---------|
| **新建 test** | 「幫我寫 unit test」 | `/unit-test create path` |
| **補充 log** | 「補充 console.log」 | `/unit-test update path` |
| **加顏色** | 「為 test 加顏色」 | `/unit-test update path` |
| **完整整合** | 「整合這個 test」 | `/unit-test update path` |
...
用戶: tests/unit/emCsvRepo.test.js
      ↓
Claude: 執行 /unit-test update tests/unit/em/emCsvRepo.test.js
      ↓
結果: ✅ 補充完成
```
...
**Q: Create 和 Update 有什麼區別？**
A:
- **Create**：從零開始根據 spec 生成全新 test file
- **Update**：改進現有 test file，補充缺失的 console.log 和顏色

**Q: 如果 test file 已經有 console.log，Update 會重複嗎？**
A: 不會。Update 會智能檢測，只補充缺失的部分。
